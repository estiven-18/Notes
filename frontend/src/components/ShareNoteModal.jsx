...
const ShareNoteModal = ({ note, onShare, onRemoveShare, onChangeRole, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    try {
      await onShare(note._id, email.trim(), role);
      setEmail("");
      setRole("editor");
    } catch (err) {
              ...
              return (
                <div key={user._id} className="shared-user-row">
                  ...
                  <select
                    ...
                    onChange={(e) => onChangeRole(note._id, user._id, e.target.value)}
                  >
                    {ROLES.map((r) => (
                      <option key={r.value} value={r.value}>{r.label}</option>
                    ))}
